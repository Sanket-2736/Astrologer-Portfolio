...
async function completeSession(bookingId: string, displayName: string, clientEmail: string, consultationType: string) {
  try {
...
export default function JitsiRoom({
  roomId, bookingId, role, displayName,
  ...
  // ── Load JaaS external_api.js ──────────────────────────────────────────
  const loadScript = useCallback((): Promise<void> => {
    if (window.JitsiMeetExternalAPI) return Promise.resolve()
    return new Promise((resolve, reject) => {
      const appId = process.env.NEXT_PUBLIC_JITSI_APP_ID
      const s = document.createElement('script')
      s.src = `https://8x8.vc/${appId}/external_api.js`  // ← JaaS script
      s.async = true
      s.onload = () => resolve()
      s.onerror = () => reject(new Error('Failed to load Jitsi. Check your internet connection.'))
  ...
  const mountJitsi = useCallback(() => {
    ...
    const api: JitsiAPI = new window.JitsiMeetExternalAPI(domain, {  // ← use domain prop
      roomName: roomId,
      jwt: jwt,                                                        // ← JWT token
      width: '100%',
      height: '100%',
      ...
      userInfo: {
        displayName: role === 'astrologer' ? `${displayName} (Astrologer)` : displayName,
      },
      configOverwrite: {
    ...
    api.addEventListener('videoConferenceLeft', doComplete)
    ...
    const durationMs = (new Date(slotEndISO).getTime() - new Date(slotStartISO).getTime()) + 15 * 60 * 1000
    const autoTimer = setTimeout(doComplete, durationMs)

    api.addEventListener('errorOccurred', (e: { error: { name: string } }) => {
      setError(`Meeting error: ${e.error?.name ?? 'unknown'}`)
    })
  ...
  }, [bookingId, clientEmail, consultationType, displayName, domain, jwt, role, roomId, slotEndISO, slotStartISO])
    ...
    return (
      ...
      >
        <div className="text-center max-w-md mystic-card p-10">
          ...
          {secsToSlot > 0 && (
            <p className="text-xs mb-6 px-3 py-2 rounded-lg inline-block"
          ...
          <div className="flex gap-3 justify-center mt-2">
            ...
            >
              ...
              <a href="/admin/bookings" className="px-6 py-3 rounded-full text-sm font-medium transition-all hover:scale-105" style={{ border: '1px solid rgba(201,168,76,0.3)', color: 'var(--gold-light)' }}>
              Cancel
            </a>
          </div>
        </div>
      ...
      <div className="flex-1 relative">
        {error && (
          <div className="absolute inset-0 flex items-center justify-center z-10 p-4">
            <p className="text-sm px-4 py-3 rounded-lg text-center max-w-sm"
              style={{ background: 'rgba(248,113,113,0.1)', color: '#f87171' }}>
              {error}
            </p>
          </div>
